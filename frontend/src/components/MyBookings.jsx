...
const MyBookings = () => {
  ...
  const handleRequireAuth = (action, booking) => {
    if (action === "login") {
      window.alert("Please log in with the same email you used to book to manage or cancel this booking.");
      navigate("/login", { state: { from: "/bookings" } });
      return;
    }
    if (action === "cancel" && booking) {
      window.alert("Log in with the same email you used to book to cancel this booking.");
      navigate("/login", { state: { from: "/bookings" } });
    }
  };
      ...
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-12">
          ...
          <p className={s.subtitle}>
            View your current and past car rental bookings
          </p>
        </div>
